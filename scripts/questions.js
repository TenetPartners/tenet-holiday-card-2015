let questions = {
  getSurveyQuestions: function() {
    return {
      q1: {
        question: 'How will Santa deliver his presents this year?',
        imageUrl: 'http://i.istockimg.com/file_thumbview_approve/36248396/5/stock-photo-36248396-blackened-cajun-sea-bass.jpg',
        options: [
          {id: 'uber', title: 'Uber', imageUrl: ''},
          {id: 'lyft', title: 'Lyft', imageUrl: ''},
          {id: 'na', title: 'Neither', imageUrl: ''}
        ]
      },
      q2: {
        question: 'What will question 2 actually be?',
        imageUrl: 'http://i.istockimg.com/file_thumbview_approve/36248396/5/stock-photo-36248396-blackened-cajun-sea-bass.jpg',
        options: [
          {id: 'uber', title: 'Uber', imageUrl: ''},
          {id: 'lyft', title: 'Lyft', imageUrl: ''},
          {id: 'na', title: 'Neither', imageUrl: ''}
        ]
      },
      q3: {
        question: 'What will question 3 actually be?',
        imageUrl: 'http://i.istockimg.com/file_thumbview_approve/36248396/5/stock-photo-36248396-blackened-cajun-sea-bass.jpg',
        options: [
          {id: 'uber', title: 'Uber', imageUrl: ''},
          {id: 'lyft', title: 'Lyft', imageUrl: ''},
          {id: 'na', title: 'Neither', imageUrl: ''}
        ]
      },
      q4: {
        question: 'What will question 4 actually be?',
        imageUrl: 'http://i.istockimg.com/file_thumbview_approve/36248396/5/stock-photo-36248396-blackened-cajun-sea-bass.jpg',
        options: [
          {id: 'uber', title: 'Uber', imageUrl: ''},
          {id: 'lyft', title: 'Lyft', imageUrl: ''},
          {id: 'na', title: 'Neither', imageUrl: ''}
        ]
      },
      q5: {
        question: 'What will question 5 actually be?',
        imageUrl: 'http://i.istockimg.com/file_thumbview_approve/36248396/5/stock-photo-36248396-blackened-cajun-sea-bass.jpg',
        options: [
          {id: 'uber', title: 'Uber', imageUrl: ''},
          {id: 'lyft', title: 'Lyft', imageUrl: ''},
          {id: 'na', title: 'Neither', imageUrl: ''}
        ]
      }
    }
  }
}

export default questions;
